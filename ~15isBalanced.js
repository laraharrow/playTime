...
function isBalanced(tree) {
  ...
  // collect depth in tree
  let depths = [];

  // store node and depth for each leaf => index 0  = node value and index 1 = depth value
  let nodes = [];
  nodes.push([tree, 0]);

  while (nodes.length) {
    // pop a node and its depth from the top of our stack stored on nodes.
    ...
    if (!node.left && !node.right) {
      // check if its a new depth and if so push it to depths array
      if (depths.indexOf(depth) < 0) {
        depths.push(depth);

        // now a tree is unbalanced if:
        // there is more then 2 leaf depth
        // 2 leaf depth that are more then 1 apart
        if (depths.length < 2 || (depth.length === 2 && Math.abs(depth[0] - depths[1]) > 1)) {
          return false;
        }
      }
      // if it isn't a leaf keep stepping down through the tree
    } else {
      if (node.left) {
...
The requirement of "the difference between the depths of any two leaf nodes is no greater than 1" implies that we'll have to compare the depths of all possible pairs of leaves. That'd be expensive—if there are n leaves, there are n^2
...
But we can also relate d to n. In a balanced tree, d is O(\log_{2}(n))O(log 2(n)). And the more unbalanced the tree gets, the closer d gets to n.
